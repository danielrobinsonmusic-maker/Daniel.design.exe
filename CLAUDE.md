...
`src/managers/AudioManager.js` plays a set of looping background layers per "area" (currently `town`, 2 layers, and `overlook`, 3 layers), switching whenever `WorldScene`/`OverlookScene` call `AudioManager.playArea(this, areaKey)` in their own `create()`. Phaser's `SoundManager` is game-global, not per-scene, so tracks keep playing uninterrupted underneath whatever building scene is on top — building scenes don't need any audio code of their own. `playArea()` no-ops when the requested area is already playing, which matters because `WorldScene.create()` re-runs every time the player walks back out of a building (see above) and would otherwise restart the town tracks from 0 on every visit. `TakeoverFrameScene` calls `AudioManager.pauseAll()`/`resumeAll()` around every content viewer so background audio (and the Workshop music-projects viewer's own YouTube embed audio) never overlap. `AudioManager.setLoopActive()` is a separate, non-area-scoped helper for condition-driven one-off loops — e.g. `WorldScene` calling it every frame with whether the player is standing in the town square, for the fountain's ambient sfx.
